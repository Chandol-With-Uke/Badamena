document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Récupérer et afficher les produits achetés
...
async function loadPurchases(userId) {
  try {
    const response = await fetch(
      `http://localhost:8080/ventee-1.0-SNAPSHOT/ressources/user/${userId}/purchases`
    );
    if (!response.ok) {
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    const purchasedProducts = await response.json();
    console.log("Voici les produits achetés:", purchasedProducts);
    ...
    purchasesContainer.innerHTML =
      '<p class="text-red-600">Erreur lors du chargement des produits achetés.</p>';
  }
}

function displayPurchases(products) {
  const purchasesContainer = document.getElementById(
    "purchasesContainer"
  );
  purchasesContainer.innerHTML = "";
  ...
  if (products.length === 0) {
    purchasesContainer.innerHTML =
      '<p class="text-gray-600">Vous n\'avez pas encore acheté de produits.</p>';
    return;
  }
  ...
  products.forEach((product) => {
    // Vous pouvez réutiliser la logique d'image de index.js si vous le souhaitez
    // Pour l'instant, utilisons un placeholder simple ou une image par défaut si vous en avez

    // Choisissez une des options ci-dessous (comme dans index.js) :
    ...
    const imageUrl =
      defaultImages[product.type] || defaultImages["Électronique"];
      ...
      <div class="p-4 flex flex-col flex-grow">
        ...
        <div class="mt-4">
          <button
            ...
            class="w-full bg-accent/90 text-white px-4 py-2 rounded-md hover:bg-accent transition duration-200"
          >
            Supprimer
          </button>
        </div>
...
// Fonction pour supprimer un produit acheté
async function removeProduct(productId) {
  const userCookie = getCookie("user");
  if (!userCookie) {
    alert("Vous devez être connecté pour supprimer un achat.");
    window.location.href = "login.html";
    return;
  }
  const user = JSON.parse(userCookie);
  ...
  try {
    ...
    if (response.ok) {
      alert("Produit supprimé de vos achats.");
      // Recharger la liste des achats après suppression
    ...
    console.error(
      "Erreur réseau ou autre lors de la suppression:",
      error
    );
...
// Fonctions utilitaires pour les cookies (copiées de index.js)
